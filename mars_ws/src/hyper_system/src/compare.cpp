...
public:
    ...
    void mapCallback(const nav_msgs::OccupancyGrid::ConstPtr& msg) {
        local_map_ = *msg;
        map_received_ = true;
        map_size_ = local_map_.info.width * local_map_.info.height;  // Default map size
    }

    void scenarioCallback(const std_msgs::String::ConstPtr& msg) {
        if (msg->data == "corridor") {
            map_size_ = static_cast<int>(3.5 * local_map_.info.height);
        } else {
            map_size_ = local_map_.info.width * local_map_.info.height;  // Reset to default map size for other scenarios
        }
    }
