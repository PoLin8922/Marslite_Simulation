...
public:
    ...
    cv::Point2d point_laser2pixel_v2(double x_from_laser, double y_from_laser, double z_from_laser);
...
ScanImageCombineNode::ScanImageCombineNode(ros::NodeHandle nh, ros::NodeHandle pnh): nh_(nh), pnh_(pnh) {
    ...
    ros::param::param<std::string>("~img_topic", img_topic, "/camera1/color/image_raw"); 
    ros::param::param<std::string>("~caminfo_topic", caminfo_topic, "/camera1/color/camera_info");
    ros::param::param<string>("~camera_frameid", camera_frameid_, "camera1_link");
    ros::param::param<bool>("~flag_det_vis", flag_det_vis_, true);
    ...
    ROS_INFO("Wait for TF from image_frame to merged_laser_frame in 10 seconds...");
    tf::StampedTransform stamped_transform;
    try{
        tf_listener_.waitForTransform(camera_frameid_, "merged_laser_frame",
                                    ros::Time(0), ros::Duration(10.0));
        tf_listener_.lookupTransform(camera_frameid_, "merged_laser_frame", 
...
double ScanImageCombineNode::calculate_distance_cost(geometry_msgs::Point location) {
    ...
    return std::hypot(location.x, location.y);
}
...
void ScanImageCombineNode::separate_outlier_points(PointCloudXYZPtr cloud_in, PointCloudXYZPtr cloud_out, bool is_far) {
    ...
    euler_extractor.setMaxClusterSize(5000);
...
cv::Point2d ScanImageCombineNode::point_laser2pixel_v2(double x_from_laser, double y_from_laser, double z_from_laser) {
    // Transform to camera frame
    tf::Vector3 pt_laserframe(x_from_laser, y_from_laser, z_from_laser); 
    tf::Vector3 pt_camframe = rot_laser2cam_ * pt_laserframe + tras_laser2cam_;
    if(pt_camframe.getZ() <= 0.0) // points behind ego
    ...
    cv::Point2d pt_cameraframe(pt_camframe.getX(), pt_camframe.getZ());

    return pt_cameraframe;
}
...
void ScanImageCombineNode::img_scan_cb(const cv_bridge::CvImage::ConstPtr &cv_ptr, const sensor_msgs::LaserScan::ConstPtr &laser_msg_ptr){
    ...
    // --------------------------------------------------------------------------------------
    // bool skip_scan_image_combine = true;
    bool skip_scan_image_combine = false;
    if(skip_scan_image_combine){
        ...
        if(pub_detection3d_.getNumSubscribers() > 0) {
            detection_array.header.frame_id = "merged_laser_frame";
    ...
    if(cloud_raw->points.size() > 0) {
        ...
        extractor.setMaxClusterSize(100);      // need to check the max pointcloud size of each object
        ...
        for(std::vector<pcl::PointIndices>::const_iterator it = cluster_indices.begin(); it != cluster_indices.end(); ++it) {
            ...
            if(laser_cluster.is_in_fov == true) {
                ...
                // test20201124
                cv::Point2d pt_uv2 = point_laser2pixel_v2(laser_cluster.location.x, laser_cluster.location.y, 0.0);
                laser_cluster.key_vec_imgspace.x = pt_uv2.x ;
                laser_cluster.key_vec_imgspace.y = pt_uv2.y ;
        ...
        if(obj_list_.size() != 0 && laser_clusters_list.size() != 0) {
            ...
            for(int i = 0; i < obj_list_.size(); i++){
                for(int j = 0; j < laser_clusters_list.size(); j++){
                    ...
                    double orientation_cost = (similarity >= 0.85)? 1.0 - similarity : 100.0; 

                    cost_matrix[i][j] = 0.4*distance_cost + orientation_cost;
            ...
            printf("obj_list_ size : %d, laser_clusters_list size : %d\n", obj_list_.size(), laser_clusters_list.size());
            HungarianAlgorithm HungAlgo;
            vector<int> assignment;
            double cost = HungAlgo.Solve(cost_matrix, assignment);
            for (unsigned int i = 0; i < cost_matrix.size(); i++){
                printf("assignment : %d ", assignment[i]+1);
            ...
            for(int i = 0; i < obj_list_.size(); i++) {
                if(obj_list_[i].cloud->points.size() >= 1){
                    printf("obj %d add to det_result !\n", i+1);
                    printf("vec image: %f  vec scan: %f\n",std::atan2(laser_clusters_list[assignment[i]].key_vec_imgspace.y, laser_clusters_list[assignment[i]].key_vec_imgspace.x) / M_PI * 180.0, std::atan2(obj_list_[i].key_vector.y, obj_list_[i].key_vector.x) / M_PI * 180.0);
                    printf("--------------------------\n");
    ...
    //     colored_cloud_msg.header.frame_id = "merged_laser_frame";
